...
const fetchWeather = createStep({
  ...
  inputSchema: z.object({
    ...
    solarSystem: z.string().describe("The solar system to get the weather for").optional(),
    ...
    constellation: z.string().describe("The constellation to get the weather for").optional(),
    zodiac: z.string().describe("The zodiac to get the weather for").optional(),
    others: z.array(z.string()).describe("Other parameters to get the weather for").optional()
  }),
...
var RESOURCE_TYPES = enumType(["agent", "workflow", "none", "tool", "none"]);
  ...
  async getRoutingAgent({ runtimeContext }) {
    ...
    const toolList = Object.entries(toolsToUse).map(([name, tool]) => {
      return ` - **${name}**: ${tool.description}, input schema: ${JSON.stringify(
        zodToJsonSchema(tool.inputSchema || objectType({}))
      )}`;
    }).join("\n");
    const instructions = `
  ...
  async loop(messages, {
    ...
    const mainWorkflow = createWorkflow({
      ...
      inputSchema: objectType({
        ...
        threadId: stringType().optional(),
        threadResourceId: stringType().optional()
      }),
      outputSchema: networkWorkflow.outputSchema
  ...
  async loopStream(messages, {
    ...
    const mainWorkflow = createWorkflow({
      ...
      inputSchema: objectType({
        ...
        threadId: stringType().optional(),
        threadResourceId: stringType().optional(),
        isOneOff: booleanType(),
        verboseIntrospection: booleanType()
      }),
      outputSchema: objectType({
        task: stringType(),
        resourceId: stringType(),
        ...
        prompt: stringType(),
        result: stringType(),
        isComplete: booleanType().optional(),
        completionReason: stringType().optional(),
        iteration: numberType()
      })
    }).dountil(networkWorkflow, async ({ inputData }) => {
      return inputData.isComplete || maxIterations && inputData.iteration >= maxIterations;
...
const agent1 = new Agent({
  name: "agent1",
  instructions: "This agent is used to do research, but not create full responses. Answer in bullet points only and be concise.",
  description: "This agent is used to do research, but not create full responses. Answer in bullet points only and be concise.",
  model: anthropic("claude-3-5-sonnet-20240620")
});
const agent2 = new Agent({
  name: "agent2",
  description: "This agent is used to do text synthesis on researched material. It writes articles in full paragraphs.",
  instructions: "This agent is used to do text synthesis on researched material. Write a full report based on the researched material. Do not use bullet points. Write full paragraphs. There should not be a single bullet point in the final report. You write articles.",
  model: anthropic("claude-3-5-sonnet-20240620")
});
...
export { Agent as A, MastraError as M, Telemetry as T, ZodFirstPartyTypeKind as Z, Tool as a, AvailableHooks as b, checkEvalStorageFields as c, executeHook as e, generateEmptyFromSchema as g, isVercelTool as i, mastra as m, registerHook as r, z };
